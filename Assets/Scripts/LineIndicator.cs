...
public class LineIndicator : MonoBehaviour
{
    ...
    private (int,int) GetSquarePosition(int square_index)
    {
        ...
        for(int row = 0; row < 9; row++)
        {
            ...
            {
                if (_lineData[row,col] == square_index)
                {
                    pos_row = row;
                    pos_col = col;
                }
    ...
    public int[] GetHorizontalLine(int square_index)
    {
        ...
        {
            line[index] = _lineData[square_position_row, index];
        }
    ...
    public int[] GetVerticalLine(int square_index)
    {
        ...
        for (int index = 0; index < 9; index++)
        {
            line[index] = _lineData[index, square_position_col];
        }
    ...
    public int[] GetSquare(int square_index)
    {
        ...
        for(int row = 0; row < 9; row++)
        {
            for(int col = 0; col < 9; col++)
            {
                if (_squareData[row, col] == square_index)
                {
                    pos_row = row;
                }
            }
        ...
        for(int index = 0; index < 9; index++)
        {
            line[index] = _squareData[pos_row, index];
        }

        return line;
    }

    public int[] GetAllSquaresIndex()
    {
